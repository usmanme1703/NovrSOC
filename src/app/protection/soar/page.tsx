    ...
    return (
        <PageLayout title="SOAR & Automation">
            <div className="space-y-5">
                <div>
                    <h1 className="text-lg font-black text-gray-900">SOAR & Automation Dashboard</h1>
                    <p className="text-xs text-gray-400">Protection & Automation · Playbook management and execution monitoring</p>
                </div>

                {/* KPIs */}
                <div className="grid grid-cols-5 gap-3">
                    {[
                        { label: 'Playbooks Today', value: SOAR_KPIS.playbooksToday, color: 'text-blue-700' },
                ...
                <div className="bg-white border border-amber-300 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs font-black text-gray-800">Active Response Queue</p>
                        </div>
                            ...
                            {ACTIVE_RESPONSES.map(r => (
                                <div key={r.name}>
                                    <div className="flex justify-between text-xs mb-1">
                                        ...
                                        <span className="text-gray-400">{r.description} — {r.pct}%</span>
                                    </div>
                ...
                {/* Playbooks */}
                <div>
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3">Playbook Library</p>
                    <div className="grid grid-cols-2 gap-3">
                        {PLAYBOOKS.map((p, i) => (
                            <div key={p.name} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-blue-200 transition-colors">
                                <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                                <div className="p-4">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0">
                                            <p className="font-bold text-gray-800 text-xs">{p.name}</p>
                                            <p className="text-[10px] text-gray-400 mt-0.5">Trigger: {p.trigger}</p>
                                        </div>
                                        <button onClick={() => { const n = [...playbookStates]; n[i] = !n[i]; setPlaybookStates(n); }}
                                            className={`ml-3 flex-shrink-0 w-10 h-5 rounded-full transition-colors relative ${playbookStates[i] ? 'bg-emerald-600' : 'bg-gray-200'}`}>
                                    ...
                                    <div className="mt-3 flex items-center justify-between">
                                        ...
                                        <button className="text-[10px] font-bold text-blue-700 border border-blue-200 px-2 py-0.5 rounded-lg hover:bg-blue-50 transition-colors">▶ Run</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                    <div className="p-4 pb-0"><p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3">Execution Log</p></div>
                            ...
                            <tbody>
                                {EXECUTION_LOG.map((e, i) => (
                                    <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-2 font-bold text-gray-800 whitespace-nowrap">{e.playbook}</td>
                                        <td className="px-4 py-2 text-gray-500 max-w-[160px] truncate">{e.trigger}</td>
                                        <td className="px-4 py-2 font-mono text-gray-700">{e.asset}</td>
                                        ...
                                        <td className="px-4 py-2 font-mono text-gray-500">{e.duration}</td>
                                        <td className="px-4 py-2 font-mono text-gray-400">{e.timestamp}</td>
                                        <td className="px-4 py-2">{e.override ? <span className="text-amber-600 text-[10px]">👤 Manual</span> : <span className="text-gray-500 text-[10px]">Auto</span>}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
