...
const CONTEXT_STATS = [
    ...
    { label: 'Top MITRE Tactic', value: 'Initial Access', color: 'text-blue-700' },
];
...
function parseAssistantMessage(text: string) {
    ...
    return (
        <div className="space-y-2 text-xs text-gray-700 leading-relaxed">
            {lines.map((line, i) => {
                if (line.startsWith('# ') || line.startsWith('## ')) return <p key={i} className="font-black text-gray-900 text-sm">{line.replace(/^#+\s/, '')}</p>;
                if (line.startsWith('- ') || line.startsWith('• ')) return <p key={i} className="flex items-start gap-2"><span className="text-blue-700 flex-shrink-0">•</span><span>{line.replace(/^[-•]\s/, '')}</span></p>;
                if (/^\d+\./.test(line)) return <p key={i} className="flex items-start gap-2"><span className="text-blue-700 font-bold flex-shrink-0">{line.match(/^\d+/)?.[0]}.</span><span>{line.replace(/^\d+\.\s/, '')}</span></p>;
            ...
            <div className="flex gap-5 h-[calc(100vh-160px)]">
                ...
                <div className="w-48 flex-shrink-0 space-y-4">
                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                        ...
                        <div className="p-3">
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">Current Stats</p>
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-200">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] text-gray-400">Live · Just now</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {messages.length > 0 && (
                        <button onClick={() => setMessages([])}
                            className="w-full text-[10px] font-bold text-gray-400 hover:text-red-600 border border-gray-200 rounded-lg py-2 transition-colors">
                            Clear Chat
                        </button>
                    )}
                </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 scrollbar-thin space-y-4">
                        {messages.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-center">
                                ...
                                <h2 className="text-base font-black text-gray-800">NovrAI</h2>
                                <p className="text-xs text-gray-400 mt-1 mb-6">Your AI-Powered SOC Analyst</p>
                                <div className="flex flex-col gap-2 w-full max-w-md">
                                    {SUGGESTED_PROMPTS.map(p => (
                                        <button key={p} onClick={() => send(p)}
                                            className="text-left text-xs text-gray-700 bg-gray-50 hover:bg-gray-100 border border-gray-200 hover:border-blue-200 rounded-lg px-4 py-2.5 transition-all">
                                            {p}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {messages.map((m, i) => (
                            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className={`max-w-[75%] rounded-xl px-4 py-3 ${m.role === 'user' ? 'bg-blue-50 border border-blue-200 text-gray-800' : 'bg-gray-50 border border-gray-200'}`}>
                                    {m.role === 'assistant' ? parseAssistantMessage(m.content) : <p className="text-xs text-gray-800">{m.content}</p>}
                                </div>
                            </div>
                        ))}
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                                    <div className="flex gap-1">
                                        {[0, 0.2, 0.4].map((delay, i) => (
                                            <div key={i} className="w-1.5 h-1.5 bg-blue-700 rounded-full animate-bounce" style={{ animationDelay: `${delay}s` }} />
                                        ))}
                                    </div>
                                </div>
                            </div>
                        ...
                        {error && <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-2">{error}</div>}
                        <div ref={bottomRef} />
                    </div>
                    ...
                    <div className="border-t border-gray-200 p-3">
                        <div className="flex gap-2 items-end">
                            <textarea
                                ...
                                rows={2}
                                className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-xs text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-700/20 resize-none"
                            />
                        ...
                        <p className="text-[9px] text-gray-500 mt-1.5">Press ⌘+Enter to send</p>
                    </div>
                </div>
            </div>
        </PageLayout>
