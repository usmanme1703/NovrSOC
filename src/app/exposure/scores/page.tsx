    ...
    return (
        <PageLayout title="Exposure Scores">
            <div className="space-y-4">
                <div>
                    <h1 className="text-lg font-black text-gray-900">Exposure Scores</h1>
                    <p className="text-xs text-gray-500">Exposure Monitoring · Risk-based asset exposure scoring and trend analysis</p>
                </div>

                {/* Overall score + trend */}
                <div className="grid grid-cols-3 gap-4">
                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex items-center gap-5 p-5">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Overall Exposure</p>
                            <p className="text-sm font-black text-orange-600 mt-1">Elevated Risk</p>
                            <p className="text-[10px] text-gray-400 mt-1">+2 from last week</p>
                        </div>
                    </div>

                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden p-5 col-span-2">
                        <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600 -mt-5 -mx-5 mb-4 rounded-t-xl" />
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">12-Month Exposure Trend</p>
                ...
                <div className="grid grid-cols-4 gap-3">
                    {[
                        { label: 'Assets Scanned', v: '1,418', color: 'text-blue-700' },
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4 border-b border-gray-100">
                        <p className="text-xs font-black text-gray-800">Asset Exposure Scores</p>
                        <p className="text-[10px] text-gray-400">Ranked by risk exposure — highest to lowest</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                            ...
                            <tbody>
                                {ASSET_SCORES.map(a => (
                                    <tr key={a.asset} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3 font-mono font-bold text-gray-800">{a.asset}</td>
                                        ...
                                        <td className={`px-4 py-3 font-black ${a.cves >= 6 ? 'text-red-600' : a.cves >= 3 ? 'text-orange-600' : 'text-gray-500'}`}>{a.cves}</td>
                                        <td className="px-4 py-3">
                                            {a.internet ? <span className="text-[10px] font-bold text-orange-600">🌐 Yes</span> : <span className="text-[10px] text-gray-400">No</span>}
                                        </td>
                                        <td className={`px-4 py-3 font-bold ${a.patches >= 4 ? 'text-red-600' : a.patches > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>{a.patches}</td>
                                        <td className="px-4 py-3 text-gray-400">{a.lastScan}</td>
                                        <td className="px-4 py-3">
                                            <button className="text-[10px] font-bold text-blue-700 hover:underline">Remediate</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
