    ...
    return (
        <PageLayout title="Analyst Workbench">
            <div className="space-y-4">
                <div>
                    <h1 className="text-lg font-black text-gray-900 dark:text-slate-100">SOC Analyst Workbench</h1>
                    <p className="text-xs text-gray-400 dark:text-slate-400">Security Operations · Full analyst toolkit — alerts, cases, threat hunting, IOC lookup</p>
                </div>

                {/* Tabs */}
                <div className="flex gap-1 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-1 w-fit">
                    {([['alerts', '⚠️ Alert Queue'], ['cases', '📂 Cases'], ['timeline', '⏱️ Timeline'], ['hunting', '🔍 Threat Hunting'], ['ioc', '🔎 IOC Lookup']] as const).map(([k, l]) => (
                        <button key={k} onClick={() => setTab(k)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${tab === k ? 'bg-blue-700 text-white' : 'text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-slate-100'}`}>{l}</button>
                    ))}
                </div>
                    ...
                    <div className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                                ...
                                <tbody>
                                    {ALERTS.map(a => (
                                        <tr key={a.id} className="border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/20 transition-colors">
                                            <td className="px-4 py-2"><span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${sevBadge[a.sev]}`}>{a.sev}</span></td>
                                            <td className="px-4 py-2 font-semibold text-gray-800 dark:text-slate-100 whitespace-nowrap">{a.name}</td>
                                            <td className="px-4 py-2 font-mono text-gray-500 dark:text-slate-400">{a.source}</td>
                                            <td className="px-4 py-2 font-mono text-gray-700 dark:text-slate-200">{a.asset}</td>
                                            <td className="px-4 py-2 font-mono text-orange-600 text-[10px]">{a.mitre}</td>
                                            <td className="px-4 py-2 font-mono text-gray-400 dark:text-slate-500">{a.time}</td>
                                            <td className="px-4 py-2"><span className="text-[10px] font-bold px-2 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 rounded">{a.status}</span></td>
                                            <td className="px-4 py-2 whitespace-nowrap">
                                                <button className="text-[10px] font-bold text-blue-700 dark:text-blue-400 hover:underline mr-2">Investigate</button>
                                                <button className="text-[10px] font-bold text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-200">Assign</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                {tab === 'cases' && (
                    <div className="space-y-3">
                        ...
                        {CASES.map(c => (
                            <div key={c.id} className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl p-4 hover:border-blue-200 dark:hover:border-blue-700/40 transition-colors">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <span className="font-mono text-[10px] text-gray-400 dark:text-slate-500">{c.id}</span>
                                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${c.priority === 'P1' ? 'bg-red-50 dark:bg-red-900/20 text-red-600' : 'bg-orange-50 dark:bg-orange-900/20 text-orange-600'}`}>{c.priority}</span>
                                        </div>
                                        <p className="font-bold text-gray-800 dark:text-slate-100 mt-1">{c.title}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Analyst: {c.analyst} · {c.alerts} linked alerts · Created: {c.created}</p>
                                    </div>
                                    <div className="flex gap-2">
                                        <span className="text-[10px] font-bold px-2 py-0.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 rounded">{c.status}</span>
                                        <button className="text-[10px] font-bold text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-700/40 px-2 py-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">View →</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                {tab === 'timeline' && (
                    <div className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                        <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                        <div className="p-4">
                            <p className="text-xs font-black text-gray-800 dark:text-slate-100 mb-1">Incident Timeline — CAS-2026-0041</p>
                            <p className="text-[10px] text-gray-400 dark:text-slate-500 mb-4">Ransomware Investigation · WORKSTATION-042</p>
                            <div className="space-y-1 border-l-2 border-blue-200 dark:border-blue-700/40 pl-4 ml-2">
                                {TIMELINE.map((t, i) => (
                                    <div key={i} className="relative pb-3">
                                        <div className="absolute -left-[21px] w-4 h-4 rounded-full bg-white dark:bg-slate-800 border-2 border-blue-200 dark:border-blue-700/40 flex items-center justify-center text-[8px]">{t.type}</div>
                                        <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 border border-gray-200 dark:border-slate-600">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-mono text-[10px] text-blue-700 dark:text-blue-400">{t.time}</span>
                                                <span className="text-xs font-bold text-gray-800 dark:text-slate-100">{t.event}</span>
                                            </div>
                                            <p className="text-[10px] text-gray-500 dark:text-slate-400">{t.detail}</p>
                                            <p className="text-[9px] text-gray-500 dark:text-slate-500 mt-0.5 font-mono">{t.asset}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {tab === 'hunting' && (
                    <div className="space-y-4">
                        <div className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                            <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                            <div className="p-4">
                                <p className="text-xs font-black text-gray-800 dark:text-slate-100 mb-3">Threat Hunting Query Builder</p>
                                <div className="space-y-2">
                                    {huntConditions.map((c, i) => (
                                        <div key={i} className="flex items-center gap-2">
                                            {i > 0 && <span className="text-[10px] font-bold text-blue-700 dark:text-blue-400 w-8">AND</span>}
                                            <select value={c.field} onChange={e => { const n = [...huntConditions]; n[i] = { ...n[i], field: e.target.value }; setHuntConditions(n); }}
                                                className="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg px-2 py-1.5 text-xs text-gray-700 dark:text-slate-200 focus:outline-none">
                                            ...
                                            <input value={c.value} onChange={e => { const n = [...huntConditions]; n[i] = { ...n[i], value: e.target.value }; setHuntConditions(n); }}
                                                className="flex-1 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-1.5 text-xs text-gray-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-700/20" />
                                            {huntConditions.length > 1 && (
                                                <button onClick={() => setHuntConditions(huntConditions.filter((_, j) => j !== i))} className="text-gray-400 dark:text-slate-500 hover:text-red-600 text-sm">✕</button>
                                            )}
                                        </div>
                                    ))}
                                    <div className="flex gap-2 mt-2">
                                        <button onClick={() => setHuntConditions([...huntConditions, { field: 'process.name', op: '=', value: '' }])} className="text-[10px] font-bold text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-700/40 px-2 py-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">+ Add Condition</button>
                        ...
                        <div className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                            <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                            <div className="p-4">
                                <p className="text-xs font-bold text-gray-500 dark:text-slate-400 mb-2">Query Results — 3 matches</p>
                                <table className="w-full text-xs">
                                    <thead><tr className="border-b border-gray-200 dark:border-slate-700">
                                        {['Timestamp', 'Host', 'Process', 'Command Line', 'User'].map(h => <th key={h} className="text-left px-3 py-2 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider whitespace-nowrap">{h}</th>)}
                                    </tr></thead>
                                    <tbody>
                                        {[['14:10:45', 'WORKSTATION-042', 'powershell.exe', 'powershell.exe -encodedCommand JAB...', 'CORP\\j.okafor'],
                                          ...
                                          ['09:28:33', 'PROD-SERVER-03', 'powershell.exe', 'powershell.exe -WindowStyle Hidden -enc SUV...', 'NT AUTHORITY\\SYSTEM'],
                                            ...
                                            <tr key={ts} className="border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/20 transition-colors">
                                                <td className="px-3 py-2 font-mono text-gray-400 dark:text-slate-500">{ts}</td>
                                                <td className="px-3 py-2 font-mono text-gray-700 dark:text-slate-200">{h}</td>
                                                <td className="px-3 py-2 font-mono text-orange-600">{p}</td>
                                                <td className="px-3 py-2 font-mono text-gray-500 dark:text-slate-400 max-w-[200px] truncate">{cmd}</td>
                                                <td className="px-3 py-2 font-mono text-gray-500 dark:text-slate-400">{u}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                {tab === 'ioc' && (
                    <div className="space-y-4">
                        <div className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                            <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                            <div className="p-4">
                                <p className="text-xs font-black text-gray-800 dark:text-slate-100 mb-3">IOC Enrichment Lookup</p>
                                <div className="flex gap-2">
                                    <input value={iocQuery} onChange={e => setIocQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && setIocResult(true)}
                                        ...
                                        className="flex-1 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg px-4 py-2.5 text-xs text-gray-800 dark:text-slate-200 placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-700/20" />
                                ...
                                {iocResult && iocQuery && (
                                    <div className="mt-4 bg-red-50 dark:bg-red-900/10 border border-red-300 dark:border-red-700/40 rounded-xl p-4 space-y-3">
                                        <div className="flex items-center justify-between">
                                            <p className="font-mono text-sm font-bold text-gray-800 dark:text-slate-100">{iocQuery}</p>
                                            <span className="text-xs font-black bg-red-50 dark:bg-red-900/20 text-red-600 border border-red-200 dark:border-red-700/40 px-3 py-1 rounded-full">🔴 MALICIOUS</span>
                                        </div>
                                        {[['VirusTotal', '48/72 engines flagged', 'Malware distribution', 'text-blue-700 dark:text-blue-400'],
                                          ...
                                          ['AlienVault OTX', '3 threat pulses', 'Linked to Lazarus Group', 'text-orange-600'],
                                            ...
                                            <div key={src} className="flex items-start gap-3 border-b border-gray-200 dark:border-slate-700/50 pb-2">
                                                <span className={`text-xs font-black ${cls} w-32 flex-shrink-0`}>{src}</span>
                                                <span className="text-xs text-gray-700 dark:text-slate-200">{det}</span>
                                                <span className="text-xs text-gray-400 dark:text-slate-500 ml-auto">{ctx}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
