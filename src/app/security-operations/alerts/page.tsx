    ...
    return (
        <PageLayout title="Alerts">
            <div className="space-y-4">
                <div>
                    <h1 className="text-lg font-black text-gray-900">Alert Queue</h1>
                    <p className="text-xs text-gray-500">Security Operations · Real-time security alert management</p>
                </div>

                {/* KPIs */}
                <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
                    {[
                        { label: 'Total Alerts', v: ALERT_KPIS.total, color: 'text-gray-900' },
                        { label: 'New Today', v: ALERT_KPIS.newToday, color: 'text-blue-700' },
                        ...
                        { label: 'Suppressed', v: ALERT_KPIS.autoSuppressed, color: 'text-gray-400' },
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {['All', 'Critical', 'High', 'Medium', 'Low'].map(s => (
                        <button key={s} onClick={() => setSevFilter(s)} className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border transition-colors ${sevFilter === s ? 'bg-blue-700 text-white border-blue-700' : 'bg-white border-gray-200 text-gray-500 hover:text-gray-700'}`}>{s}</button>
                    ))}
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                        className="text-[10px] font-bold bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-gray-500 focus:outline-none">
                        <option>All</option>
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                            ...
                            <tbody>
                                {filtered.map(a => (
                                    <tr key={a.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${selected.includes(a.id) ? 'bg-blue-50/50' : ''}`}>
                                        ...
                                        <td className="px-4 py-2.5 font-semibold text-gray-800 max-w-[180px] truncate">{a.name}</td>
                                        <td className="px-4 py-2.5 font-mono text-gray-500 text-[10px]">{a.source}</td>
                                        <td className="px-4 py-2.5 font-mono text-gray-700 text-[10px]">{a.asset}</td>
                                        <td className="px-4 py-2.5 font-mono text-orange-600 text-[10px]">{a.mitre}</td>
                                        <td className="px-4 py-2.5 text-gray-500 text-[10px]">{a.tactic}</td>
                                        <td className="px-4 py-2.5 font-mono text-gray-400 text-[10px]">{a.time}</td>
                                        <td className="px-4 py-2.5">
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${STATUS_BADGE[a.status]}`}>{a.status}</span>
                                        </td>
                                        <td className="px-4 py-2.5 whitespace-nowrap">
                                            <div className="flex gap-2">
                                                <button className="text-[10px] font-bold text-blue-700 hover:underline">Assign</button>
                                                <button className="text-[10px] font-bold text-gray-400 hover:text-gray-700">Suppress</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
                        <p className="text-[10px] text-gray-400">Showing {filtered.length} of {ALERTS.length} alerts</p>
                        <div className="flex gap-1">
                            {[1, 2, 3].map(p => (
                                <button key={p} className={`w-6 h-6 text-[10px] font-bold rounded ${p === 1 ? 'bg-blue-700 text-white' : 'text-gray-500 hover:bg-gray-100'}`}>{p}</button>
                            ))}
                        </div>
                    </div>
                </div>
