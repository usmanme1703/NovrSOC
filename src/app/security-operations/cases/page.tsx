        ...
        <PageLayout title="Cases">
            <div className="space-y-4">
                <div className="flex items-start justify-between">
                    <div>
                        <h1 className="text-lg font-black text-gray-900">Case Management</h1>
                        <p className="text-xs text-gray-500">Security Operations · Incident case tracking and investigation</p>
                    </div>
                ...
                <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
                    {[
                        { label: 'Total Cases', v: CASE_KPIS.total, color: 'text-gray-900' },
                        { label: 'Open', v: CASE_KPIS.open, color: 'text-gray-600' },
                                    ...
                                    <tbody>
                                        {CASES.map(c => (
                                            <tr key={c.id} onClick={() => setSelected(selected === c.id ? null : c.id)}
                                                className={`border-b border-gray-100 cursor-pointer transition-colors ${selected === c.id ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                                                <td className="px-4 py-3 font-mono text-blue-700 font-bold text-[10px]">{c.id}</td>
                                                ...
                                                <td className="px-4 py-3 text-gray-600">{c.analyst}</td>
                                                <td className="px-4 py-3 text-center font-black text-gray-700">{c.linkedAlerts}</td>
                                                <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{c.created}</td>
                                                <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{c.updated}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    {detail && (
                        <div className="col-span-1 space-y-3">
                            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                                ...
                                <div className="p-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <p className="text-[10px] font-mono text-blue-700">{detail.id}</p>
                                        <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-gray-600 text-xs">✕</button>
                                    </div>
                                    <h3 className="text-sm font-black text-gray-900 mb-2">{detail.title}</h3>
                                    <p className="text-[10px] text-gray-500 mb-3">{detail.description}</p>
                                    ...
                                    <div className="space-y-2">
                                        <select className="w-full text-[10px] bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-gray-700 focus:outline-none">
                                        ...
                                        <button className="w-full py-2 border border-gray-200 text-gray-600 text-[10px] font-bold rounded-lg hover:bg-gray-50 transition-colors">Add Note</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                                <div className="h-[3px] bg-gradient-to-r from-blue-700 via-violet-600 to-red-600" />
                                <div className="p-4">
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3">Case Timeline</p>
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            <div key={i} className="flex gap-3 text-[10px]">
                                                <span className="font-mono text-gray-400 flex-shrink-0 w-10">{e.time}</span>
                                                <div>
                                                    <p className="font-semibold text-gray-700">{e.event}</p>
                                                    <p className="text-gray-400">{e.actor}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md shadow-2xl">
                        ...
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-sm font-black text-gray-900">New Case</h3>
                            ...
                            <div className="space-y-3">
                                {[
                                    ...
                                    <div key={f.label}>
                                        ...
                                        {f.type === 'textarea'
                                            ? <textarea placeholder={f.ph} rows={3} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-700/20" />
                                            : <input type="text" placeholder={f.ph} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-700/20" />
                                        }
                                    </div>
                                ))}
                                <div>
                                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1">Priority</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1">Assign To</label>
                                    <select className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 focus:outline-none">
